...
const BottomNav = () => {
  ...
  // Distribute uniquement si un item central existe
  const itemsPerSide = centralItem ? Math.floor(mainNavItems.length / 2) : 0;
  const leftItems = centralItem ? mainNavItems.slice(0, itemsPerSide) : [];
  const rightItems = centralItem ? mainNavItems.slice(itemsPerSide) : [];
      ...
      >
        <div className="container mx-auto px-2 h-full">
          <div className="flex items-center justify-around h-full pt-2 pb-3 bottom-nav-inner">
            {(centralItem ? leftItems : mainNavItems).map(({ path, icon: Icon, label }) => {
            ...
            {(centralItem ? rightItems : []).map(({ path, icon: Icon, label }) => {
              const isActive = isPathActive(path);
              return (
                <Link
                  key={path}
