...
const CommentSection = ({ postId, postOwnerId, authorName, postContent, audioParentId, refreshBalance, highlightCommentId }) => {
  ...
  const handleAddComment = async (e) => {
    ...
    try {
        ...
        let safeContent = (newComment || '').replace(/(^|[^\w@])@([A-Za-z0-9À-ÖØ-öø-ÿ'’._-]{1,30})(?=$|[^A-Za-z0-9À-ÖØ-öø-ÿ'’._-])/g, '$1[[m:$2]]');
...
const AudioPostCard = ({ post, user, profile, onDelete, onWarn, refreshBalance, autoOpen, highlightCommentId }) => {
  ...
  const checkLiked = useCallback(async () => {
    try {
      if (!post?.id) return;
      // statut like par l'utilisateur (via content_id)
      if (user?.id) {
      ...
      // compteur de likes (via content_id)
      const { count } = await supabase
        .from('likes')
        .select('id', { count: 'exact', head: true })
        .eq('content_id', post.id)
      ...
      <CardContent className="pt-6">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <AudioPlayer src={post.audio_url} initialDuration={post.audio_duration} />
            <div className="flex items-center flex-wrap gap-2 sm:gap-4 text-[#6B6B6B] mt-3">
              <button
                className={`flex items-center gap-1 hover:text-[#E0222A] transition-colors ${isLiked ? 'text-[#E0222A]' : ''}`}
                onClick={handleLike}
              >
                <Heart className={`h-5 w-5 ${isLiked ? 'fill-current' : ''}`} />
              ...
              <button
                className="flex items-center gap-1 hover:text-[#2BA84A] transition-colors"
                onClick={() => setCommentsOpen((v) => !v)}
              >
                <MessageCircle className="h-5 w-5" />
                <span>{commentsCount || 0}</span>
              </button>
              <button className="flex items-center gap-1 hover:text-[#007AFF] transition-colors" onClick={handleShare}>
                <Share2 className="h-5 w-5" />
                <span>Partager</span>
              </button>
              {!isAndroidNativeApp && (
              ...
              {user && !isMyPost && (
                <DonationDialog
                  ...
                  refreshBalance={refreshBalance}
                >
                  <button className="flex items-center gap-1 hover:text-[#F5C300] transition-colors ml-2 sm:ml-auto sm:shrink-0">
                    <Coins className="h-5 w-5" />
                    <span>Don</span>
                  </button>
                </DonationDialog>
